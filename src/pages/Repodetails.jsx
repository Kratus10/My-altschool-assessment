...
function RepoDetails() {
  ...
  useEffect(() => {
    fetch(`https://api.github.com/users/kratus10/repos/${id}`)
  ...
  useEffect(() => {
    fetch(`https://api.github.com/users/kratus10/repos/${id}/branches`)
    .then((response) => (response.json()))
    .then((data) => {
      setBranch(data)
    })
  ...
  useEffect(() => {
    fetch(`https://api.github.com/users/kratus10/repos/${id}/deployments`)
    .then((response) => (response.json()))
    .then((data) => {
      setDeployment(data)
    })
  ...
  return (
    <div id="repodetail">
        <div className="repodetail-card">
            ...
            <p>Live site: {deployment.length === 0 ? `none` : <a href={`https://github.com/Kratus10/${details.name}`}>github.com/Kratus10/{details.name}</a>}</p>
